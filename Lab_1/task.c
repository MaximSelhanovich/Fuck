...
void evenPalindroms(FILE* fileToRead) {
    ...
    while (currentPosition < maxPosition) {
        while (currentPosition + rightMove <= maxPosition &&
            ...
            //w hile
            while ((!isCharacter(innerLeftChar)) && (ftell(fileToRead) >= 0)) {
                fseek(fileToRead, -3, SEEK_CUR);
                outerLeftChar = fgetc(fileToRead);
                innerLeftChar = fgetc(fileToRead);
                ++leftMove;
            }
           ...
           while (!isCharacter(innerRightChar) && innerRightChar != EOF) {
                ++rightMove;
                innerRightChar = fgetc(fileToRead);
           }

            if (ftell(fileToRead) == maxPosition) outerRightChar = ' ';
            else outerRightChar = fgetc(fileToRead);
            /**/

            if (isCharacter(outerLeftChar) && isCharacter(outerRightChar) &&
                outerLeftChar != outerRightChar) {
                break;
            }

            if (!isCharacter(outerLeftChar) && !isCharacter(outerRightChar) &&
                isCharacter(innerLeftChar) && innerLeftChar == innerRightChar) {
